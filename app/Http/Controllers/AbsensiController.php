  ...
  public function store(Request $request)
{
    ...
    // Ambil ID guru berdasarkan user login
    ...
    // Ambil semua siswa yang memang berada di kelas yang dipilih
    $siswaKelas = \App\Models\Siswa::where('kelas_id', $request->kelas_id)
        ->whereIn('id', $request->siswa_ids)
        ->pluck('id');

    if ($siswaKelas->isEmpty()) {
        return back()->with('error', 'Tidak ada siswa yang valid untuk kelas ini.');
    }

    // Simpan absensi hanya untuk siswa yang sesuai kelas
    foreach ($siswaKelas as $siswaId) {
    ...
    return redirect()->route('guru.absensi.index')->with('success', 'Absensi berhasil disimpan untuk siswa kelas yang dipilih.');
}
