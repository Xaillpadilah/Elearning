...
<head>
  ...
  @vite(['resources/css/adminguru.css'])
    @vite(['resources/css/ penilaian.css'])
  ...
  <style>
    ...
    .input-nilai-form {
      width: 100%;
      min-width: 800px;
      /* agar tetap readable, tapi scrollable di layar kecil */
      border-collapse: collapse;
    }
...
<body>
  <div class="sidebar">
    ...
    <ul>
      <li><a href="{{ route('guru.dashboard') }}" class="{{ request()->routeIs('guru.dashboard') ? 'active' : '' }}">🏠
          Dashboard</a></li>
      <li><a href="{{ route('materi.index') }}" class="{{ request()->routeIs('materi.index') ? 'active' : '' }}">📚
          Materi dan Konten</a></li>
      <li><a href="{{ route('guru.menu') }}" class="{{ request()->routeIs('guru.menu') ? 'active' : '' }}">📝 Kuis dan
          Tugas</a></li>
    ...
    <div class="card-container">
      @foreach($relasiGuruMapelKelas as $relasi)
        <div class="card-kelas">
          ...
          <button class="btn-input" style="margin-left: 10px; background: linear-gradient(to right, #007bff, #0056b3);"
                  onclick="toggleNilai({{ $relasi->id }})">📋 Lihat Nilai Siswa</button>
                  ...
                  <tbody>
                    @foreach ($penilaiansFiltered as $i => $p)
                      <tr>
                        ...
                        <td><input type="number" step="0.01" name="nilai[{{ $i }}][tugas]" class="form-control" value="{{ $p->nilai_tugas }}"></td>
                        <td><input type="number" step="0.01" name="nilai[{{ $i }}][kuis]" class="form-control" value="{{ $p->nilai_kuis }}"></td>
                        <td><input type="number" step="0.01" name="nilai[{{ $i }}][uts]" class="form-control" value="{{ $p->nilai_uts }}"></td>
                        <td><input type="number" step="0.01" name="nilai[{{ $i }}][uas]" class="form-control" value="{{ $p->nilai_uas }}"></td>
                        <td><input type="text" name="nilai[{{ $i }}][catatan]" class="form-control" value="{{ $p->catatan }}"></td>
                      </tr>
                    @endforeach
                  </tbody>
                </table>
...
<!-- MODALS DIPISAH DI LUAR MAIN CONTAINER -->
@foreach($relasiGuruMapelKelas as $relasi)
